  ...
  return (
    <div className="terminal-window rounded-lg p-6 font-tech text-sm">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="ml-2 text-hacker-green-dim">ANONYMIKETECH Terminal</span>
      </div>
      ...
      <div className="space-y-1">
        ...
        {currentLine < terminalLines.length && (
          <div className="text-hacker-green-bright">
            {currentText}
            {showCursor && <span className="inline-block w-2 h-4 bg-hacker-green ml-1 animate-blink"></span>}
          </div>
        )}

        {isComplete && <div className="text-hacker-green glow-text mt-4 animate-flicker">█ SYSTEM READY █</div>}
      </div>
    </div>
  )
}
